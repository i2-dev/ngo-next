...
async function getPreviewSuccessCaseData(locale = 'en', status = 'draft', pLevel = 5) {
  try {
    ...
    if (result?.data) {
      // 處理排序並找到 order === 5 的案例 (i-Change賭博輔導平台)
      ...
      // 找到 i-Change賭博輔導平台案例 (order === 5)
      const iChangeCase = sortedCases.find(successCase => successCase.order === 5);
      
      return {
        successCase: iChangeCase,
...
export default function IChangeGamblingCounselingPreview() {
  ...
  return (
    <PreviewWrapper
      contentType="i-change-gambling-counseling"
      loading={loading}
      error={error}
      data={data}
    >
