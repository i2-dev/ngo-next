...
async function getPreviewSuccessCaseData(locale = 'en', status = 'draft', pLevel = 5) {
  try {
    ...
    if (result?.data) {
      ...
      // 找到智有腦健腦遊戲系統案例 (order === 2)
      const brainTrainingCase = sortedCases.find(successCase => successCase.order === 2);
      
      return {
        successCase: brainTrainingCase,
